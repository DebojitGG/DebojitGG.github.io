...
class OAuth2Client
{
	...
	public $curl_useragent           = "OAuth/2 Simple PHP Client v0.1; SocialIntegrationAuth http://hybridauth.sourceforge.net/";
	...
	*/
    ...
    {
        $response = $this->request( $this->revoke_token_url, array('token' => $token), "GET" );
        return $this->parseRequestResult( $response );         
    }
	...
	private function request( $url, $params=false, $type="GET" )
	{
		...
		// SocialIntegration_Logger::info( "Enter OAuth2Client::request( $url )" );
		// SocialIntegration_Logger::debug( "OAuth2Client::request(). dump request params: ", serialize( $params ) );
		...
		if ( $this->access_token && isset($params['authorization_header'])){
		...
		if( $response === FALSE ) {
				// SocialIntegration_Logger::error( "OAuth2Client::request(). curl_exec error: ", curl_error($ch) );
		}
		// SocialIntegration_Logger::debug( "OAuth2Client::request(). dump request info: ", serialize( curl_getinfo($ch) ) );
		// SocialIntegration_Logger::debug( "OAuth2Client::request(). dump request result: ", serialize( $response ) );
