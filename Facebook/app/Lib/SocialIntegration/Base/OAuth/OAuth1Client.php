...
class OAuth1Client{
	...
	public $curl_useragent        = "OAuth/1 Simple PHP Client v0.1; SocialIntegrationAuth http://hybridauth.sourceforge.net/";
	...
	*/ 
	...
	{
		// SocialIntegration_Logger::info( "Enter OAuth1Client::request( $method, $url )" );
		// SocialIntegration_Logger::debug( "OAuth1Client::request(). dump post fields: ", serialize( $postfields ) );
		...
		if( $response === false ) {
				//SocialIntegration_Logger::error( "OAuth1Client::request(). curl_exec error: ", curl_error($ci) );
		}
