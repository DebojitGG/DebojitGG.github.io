...
class Hybrid_Endpoint {
	...
	protected function processOpenidXRDS()
	{
		...
		(
			...
			str_replace(
				array("<", ">", "\"", "'", "&"), array("&lt;", "&gt;", "&quot;", "&apos;", "&amp;"),
				SocialIntegration_Auth::getCurrentUrl( false )
			),
	...
	protected function processOpenidRealm()
	{
		...
		(
			"{X_XRDS_LOCATION}",
			htmlentities( SocialIntegration_Auth::getCurrentUrl( false ), ENT_QUOTES, 'UTF-8' ) . "?get=openid_xrds&v=" . SocialIntegration_Auth::$version,
	...
	protected function processAuthStart()
	{
		...
		if( ! SocialIntegration_Auth::storage()->get( "hauth_session.$provider_id.hauth_endpoint" ) ) {
		...
		$hauth = SocialIntegration_Auth::setup( $provider_id );
	...
	protected function authInit()
	{
		if ( ! $this->initDone) {
			...
			try {
				...
				Hybrid_Logger::error( "Endpoint: Error while trying to init SocialIntegration_Auth: " . $e->getMessage());
