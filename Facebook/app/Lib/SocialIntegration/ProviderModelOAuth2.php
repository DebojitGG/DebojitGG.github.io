...
class SocialIntegration_Provider_Model_OAuth2 extends SocialIntegration_Provider_Model
{ 
	/**
	 * default permissions
	 * @var string
	...
	*/
	function initialize() 
	{
		...
		require_once SocialIntegration_Auth::$config["path_libraries"] . "OAuth/OAuth2Client.php";
	...
	*/
	function loginBegin()
	{
		...
		SocialIntegration_Auth::redirect( $this->api->authorizeUrl( array( "scope" => $this->scope ) ) ); 
	}
		...
		if( $this->api->access_token ){
			...
			// if( $this->api->refresh_token && $this->api->access_token_expires_at ){
				...
				if( $this->api->access_token_expires_at <= time() ){ 
					$response = $this->api->refreshToken( array( "refresh_token" => $this->api->access_token ) );
