...
class Hybrid_Provider_Model_OpenID extends Hybrid_Provider_Model
{
	...
	*/
	function initialize()
	{
		...
		require_once SocialIntegration_Auth::$config["path_libraries"] . "OpenID/LightOpenID.php";

		// An error was occurring when proxy wasn't set. Not sure where proxy was meant to be set/initialized.
		SocialIntegration_Auth::$config['proxy'] = isset(SocialIntegration_Auth::$config['proxy'])?SocialIntegration_Auth::$config['proxy']:'';

		$hostPort = parse_url( SocialIntegration_Auth::$config["base_url"], PHP_URL_PORT);
		$hostUrl = parse_url( SocialIntegration_Auth::$config["base_url"], PHP_URL_HOST);
		...
		$this->api = new LightOpenID( $hostUrl, SocialIntegration_Auth::$config["proxy"] );
	}
	...
	*/
	function loginBegin()
	{
		...
		SocialIntegration_Auth::redirect( $this->api->authUrl() );
	}
	...
	*/
	function loginFinish()
	{
		...
		SocialIntegration_Auth::storage()->set( "hauth_session.{$this->providerId}.user", $this->user );
	}
	...
	*/
	function getUserProfile()
	{
		...
		$this->user = SocialIntegration_Auth::storage()->get( "hauth_session.{$this->providerId}.user" ) ;
